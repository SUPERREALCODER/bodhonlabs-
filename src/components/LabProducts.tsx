...
const labProducts: Product[] = [
  {
    ...
    icon: ShieldAlert,
    tags: ["REACT NATIVE", "FOCUS MODE", "ALGORITHM BLOCKER"],
    links: {
      web: "#",
      playStore: "#",
      apk: "#",
  ...
  },
  {
    ...
    icon: BrainCircuit,
    tags: ["LOCAL LLM", "AGENTIC AI", "DATA PRIVACY"],
    links: {
      web: "#",
      playStore: "#",
      apk: "#",
...
export default function LabProducts() {
  return (
    <section className="py-24 bg-[#05050A] text-white relative overflow-hidden min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          >
            ...
            <span className="text-[10px] font-mono uppercase tracking-widest text-slate-300">Internal Products</span>
          </motion.div>
          <motion.h2 
          ...
          {labProducts.map((product, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative p-8 md:p-10 rounded-[2.5rem] bg-[#0F1117]/80 backdrop-blur-xl border border-white/5 ${product.themeColor.border} transition-all duration-500 flex flex-col h-full shadow-2xl shadow-black/50`}
              >
                ...
                <div className="relative z-10 flex-grow">
                  <div className="flex justify-between items-start mb-8">
                    <div 
                      className={`w-16 h-16 rounded-2xl bg-[#1A1C23] border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 shadow-lg`}
                    ...
                    <div className="flex flex-wrap justify-end gap-2 max-w-[50%]">
                      {product.tags.map((tag) => (
                        <span 
                          key={tag} 
                          className="px-2.5 py-1 text-[9px] font-mono tracking-wider uppercase text-slate-500 border border-white/5 rounded-md bg-white/5 whitespace-nowrap"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <h3 className={`text-3xl font-bold mb-4 tracking-tight transition-colors duration-300 ${product.themeColor.text.replace('text-', 'group-hover:text-')}`}>
                    {product.name}
                  </h3>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                      <a 
                        ...
                        className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 ${product.themeColor.primary} hover:brightness-110 text-white text-sm font-bold rounded-xl transition-all shadow-lg active:scale-95`}
                      >
                        <Globe size={18} />
                        Live Preview
                      </a>
                      ...
                      <a 
                        ...
                        className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-[#1A1C23] hover:bg-[#252833] border border-white/10 text-white text-sm font-semibold rounded-xl transition-all active:scale-95 group/play"
                      >
